...
namespace ProjectPerson.Service.Service
{
    public class PersonService : IPersonService
    {
        ...
        public async Task<IEnumerable<PersonDTO>> GetDeliverer()
        {
            IEnumerable<Person> persons = await _personRepository.GetPersonByType(Enums.PersonType.Deliverer);
            if (persons == null)
            {
                throw new KeyNotFoundException("User does not exists.");
            }
        ...
        private async Task<string> SaveImage(IFormFile imageFile)
        {
            ...
            var imagePath = Path.Combine(Directory.GetCurrentDirectory(), @"..\ProjectPerson.Common\Images", imageName);
